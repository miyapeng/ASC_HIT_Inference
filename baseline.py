...
'''
python baseline.py --dataset scrambled_sampled_dataset.json --model llama2-70b-hf --num-samples=10
'''
...
def run_hf(
    ...
    llm = AutoModelForCausalLM.from_pretrained(
        model, device_map = "auto" , torch_dtype=torch.float16, trust_remote_code=trust_remote_code)
    ...
    for i in tqdm(range(len(requests))):
        ...
        generated_text = tokenizer.decode(llm_outputs[0], skip_special_tokens=True)
        # print(f"Prompt: {prompt!r}, Generated text: {generated_text!r}")
        input_num_tokens.append(len(input_ids[0]))
        output_num_tokens.append(len(llm_outputs[0]))
        data_to_save.append((generated_text, len(llm_outputs[0])-len(input_ids[0])))
